...
class AlienInvasion:
    ...
    def __init__(self):
        ...
        # Create instance of the game stats
        # and create a scoreboard
        self.stats = GameStats(self)
        self.sb = Scoreboad(self)
    ...
    def _check_bullet_alien_collisions(self):
        """Responds to any bullet alien collisions"""
        # Remove any bullets and aliens that have collided
        # Checking for collisions
        # If collision, get rid of bullet and the corresponding alien then add the score to the score total
        collisions = pygame.sprite.groupcollide(self.bullets, self.aliens, True, True)

        if collisions:
            # Dealing with collision of multiple aliens by a single bullet
    ...
    def _ship_hit(self):
        ...
        if self.stats.ships_left > 0:
            # Decrement ships avail
            self.stats.ships_left -= 1

            # Get rid of any remanin aliens and bullets
            self.aliens.empty()
            self.bullets.empty()
    ...
    def _check_aliens_bottom(self):
        ...
        for alien in self.aliens.sprites():
            if alien.rect.bottom >= screen_rect.bottom:
                # Treat this the same as an end game scenario
                self._ship_hit()
                break

    def _check_play_button(self, mouse_pos):
        """Start a new game when the mouse click is over and clicks the play button"""
